...
<div class="page-header d-flex justify-content-between align-items-start">
    ...
    @if($unreadCount > 0)
    <form action="{{ route('guru.notifikasi.mark-all-read') }}" method="POST" class="d-inline">
        @csrf
            ...
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr class="{{ $n->is_read ? '' : 'table-active' }}" style="{{ $n->is_read ? '' : 'font-weight: 600;' }}">
                        ...
                        <td>
                            @if($n->link)
                            <a href="{{ route('guru.notifikasi.mark-read', $n) }}" class="btn btn-sm" style="background: var(--primary-50); color: var(--primary-700);" title="Lihat">
                                <i class="bi bi-arrow-right"></i>
                            </a>
                            @else
                            <form action="{{ route('guru.notifikasi.mark-read', $n) }}" method="POST" class="d-inline">
                                @csrf
