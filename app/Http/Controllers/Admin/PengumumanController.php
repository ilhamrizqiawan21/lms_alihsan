...
class PengumumanController extends Controller
{
            ...
            $query->where(function($q) use ($guruKelasIds) {
                $q->whereIn('target',['semua','guru'])->orWhere('created_by',Auth::id())->orWhere(function($q) use ($guruKelasIds) {
                    $q->where('target','kelas_mapel')->where(function($q) use ($guruKelasIds) {
                        ...
                        foreach ($guruKelasIds as $kelasId) $q->orWhere('target_kelas','like','%"'.$kelasId.'"%');
                    });
                });
            });
        }
    ...
    public function show(Pengumuman $pengumuman){$role=Auth::user()->role?->nama_role;abort_unless($this->canView($pengumuman,$role),403);$pengumuman->loadMissing(['creator','kelasMapel.kelas','kelasMapel.mataPelajaran']);$targetKelasLabels=Kelas::whereIn('id',$pengumuman->targetKelasIds())->orderBy('tingkat')->orderBy('nama_kelas')->get()->map(fn(Kelas $k)=>trim($k->tingkat.' '.$k->nama_kelas));return Inertia::render('Admin/Pengumuman/Show',compact('pengumuman','targetKelasLabels')+['routePrefix'=>$this->routePrefix()]);}
    public function store(Request $request){$role=Auth::user()->role?->nama_role;$allowed=match($role){'guru'=>['kelas_mapel'],'admin','kepala_sekolah'=>['semua','guru','siswa','kelas_mapel'],default=>[]};abort_unless($allowed!==[],403);$v=$request->validate(['judul'=>'required|string|max:200','isi'=>'required|string','target'=>['required',Rule::in($allowed)],'target_kelas_ids'=>'nullable|required_if:target,kelas_mapel|array','target_kelas_ids.*'=>'integer|exists:kelas,id']);$v=$this->prepareTarget($v);$v['created_by']=Auth::id();Pengumuman::create($v);return redirect()->route($this->routePrefix().'.index')->with('success','Pengumuman berhasil dipublikasikan.');}
    public function update(Request $request,Pengumuman $pengumuman){$role=Auth::user()->role?->nama_role;abort_unless($role==='admin'||(int)$pengumuman->created_by===(int)Auth::id(),403);$allowed=match($role){'guru'=>['kelas_mapel'],'admin','kepala_sekolah'=>['semua','guru','siswa','kelas_mapel'],default=>[]};$v=$request->validate(['judul'=>'required|string|max:200','isi'=>'required|string','target'=>['required',Rule::in($allowed)],'target_kelas_ids'=>'nullable|required_if:target,kelas_mapel|array','target_kelas_ids.*'=>'integer|exists:kelas,id']);$pengumuman->update($this->prepareTarget($v));return redirect()->route($this->routePrefix().'.index')->with('success','Pengumuman berhasil diperbarui.');}
    public function destroy(Pengumuman $pengumuman){$role=Auth::user()->role?->nama_role;abort_unless($role==='admin'||(int)$pengumuman->created_by===(int)Auth::id(),403);$pengumuman->delete();return redirect()->route($this->routePrefix().'.index')->with('success','Pengumuman berhasil dihapus.');}
