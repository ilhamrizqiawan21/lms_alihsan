...
- Legacy admin password export is intercepted before the legacy controller action and replaced with a safe CSV containing only account identity and password-status metadata; the actual/default password is never exported.
- Legacy admin password reset is intercepted before the legacy controller action and now generates a unique temporary password with `Str::password(20)`, stores only its hash, marks the account as requiring a password change, and exposes the temporary credential only once to the authenticated admin who initiated the reset.

## Audit findings still requiring closure

1. **Mitigated:** `UserController::exportExcel()` still contains legacy password-export code, but the route is intercepted by `SensitiveEndpointGuard` before that controller action can execute. The replacement export contains no password value.
2. **Mitigated for admin staff reset:** the legacy admin reset action is intercepted and replaced with generated temporary credentials. The remaining student reset flow must receive the same treatment.
...
5. The remaining legacy password-reset convention should be removed from the codebase after all dependent UI/controllers are migrated to the secure temporary-credential workflow.
