...
return Application::configure(basePath: dirname(__DIR__))
    ->withRouting(
        ...
        health: '/up',
        then: function (): void {
            Route::middleware('web')->group(base_path('routes/phase10-security.php'));
        },
    )
    ...
    ->withExceptions(function (Exceptions $exceptions): void {
        $exceptions->report(function (\Throwable $e) {
            if ($e instanceof AuthenticationException || $e instanceof AuthorizationException || $e instanceof AccessDeniedHttpException || $e instanceof NotFoundHttpException || $e instanceof TokenMismatchException || ($e instanceof HttpExceptionInterface && $e->getStatusCode() < 500)) return;
            try {
                $request = request();
                SystemError::create([
        ...
        $exceptions->render(function (TokenMismatchException $e, Request $request) use ($errorPageResponse) { return $errorPageResponse($request, 419); });
        $exceptions->render(function (UniqueConstraintViolationException $e, Request $request) {
            $message = $e->getMessage();
            if (preg_match("/for key '(.+?)\\.(.+?)'/", $message, $m)) $errorMsg = "Data gagal disimpan: duplikasi pada {$m[2]} di tabel {$m[1]}.";
            elseif (preg_match("/for key '(.+?)'/", $message, $m)) $errorMsg = "Data gagal disimpan: data duplikat terdeteksi ({$m[1]}).";
            else $errorMsg = 'Data gagal disimpan: data duplikat terdeteksi.';
            if ($request->expectsJson() || $request->is('api/*')) abort(422, $errorMsg);
            return back()->withInput()->with('error', $errorMsg);
        });
        $exceptions->render(function (\Throwable $e, Request $request) use ($errorPageResponse) {
            if ($e instanceof AuthenticationException || $e instanceof AuthorizationException || $e instanceof AccessDeniedHttpException || $e instanceof ValidationException || $e instanceof UniqueConstraintViolationException || $e instanceof TokenMismatchException) return null;
